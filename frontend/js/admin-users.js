  ...
  $tbody.innerHTML = users.map((user) => `
    <tr>
      <td>
        ...
        ${user.username === session?.username ? '<div class="table-subtext">Tài khoản hiện tại</div>' : ""}
      </td>
      <td>
      ...
      <td>
        <div class="inline-field">
          <input type="password" minlength="6" maxlength="100" placeholder="Mật khẩu mới" data-password-input="${user.id}" />
...
async function loadUsers() {
  setStatus($status, "Đang tải danh sách người dùng...", "info");
  users = await apiGet("/admin/users");
  renderUsers();
  setStatus($status, `Đã tải ${users.length} người dùng.`, "success");
}
...
async function handleTableClick(event) {
  ...
  try {
    if (button.dataset.action === "save-role") {
      ...
      setStatus($status, `Đã cập nhật role cho ${user.username}.`, "success");
      await loadUsers();
      return;
    }
    ...
    if (button.dataset.action === "reset-password") {
      ...
      if (newPassword.length < 6) {
        setStatus($status, "Mật khẩu mới phải có ít nhất 6 ký tự.", "error");
        return;
      }

      await apiPost(`/admin/users/${userId}/password`, { newPassword });
      setStatus($status, `Đã đặt lại mật khẩu cho ${user.username}.`, "success");
      await loadUsers();
      return;
    }
    ...
    setStatus($status, `Đã xóa user ${user.username}.`, "success");
    await loadUsers();
  } catch (error) {
    console.error(error);
