...
$forgotForm.addEventListener("submit", async (event) => {
  ...
  if (newPassword !== confirmPassword) {
    setStatus($forgotStatus, "Mat khau xac nhan khong khop.", "error");
  ...
  try {
    ...
    setStatus($forgotStatus, response.message || "Da cap nhat mat khau. Ban co the dang nhap lai.", "success");
    $forgotForm.reset();
  } catch (error) {
    console.error(error);
